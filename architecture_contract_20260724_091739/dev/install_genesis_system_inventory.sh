...
cd "${PROJECT_ROOT}"
mkdir -p dev/verification docs/audits

cat > dev/verification/inventory_genesis_system.py <<'PYEOF'
#!/usr/bin/env python3
...
cat > dev/inventory_genesis_system.sh <<'SHEOF'
#!/usr/bin/env bash
set -euo pipefail
...
cd "${PROJECT_ROOT}"
"${PYTHON_BIN}" dev/verification/inventory_genesis_system.py
SHEOF

chmod +x dev/inventory_genesis_system.sh dev/verification/inventory_genesis_system.py

printf '\nInstalled:\n  dev/inventory_genesis_system.sh\n  dev/verification/inventory_genesis_system.py\n\n'
"${PYTHON_BIN}" dev/verification/inventory_genesis_system.py
