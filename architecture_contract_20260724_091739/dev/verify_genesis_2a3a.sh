...
for manifest in (
    ...
    for raw in manifest.read_text(encoding="utf-8").splitlines():
        ...
        assert not line.startswith("/"), (
            f"absolute path in {manifest}: {line}"
        )
        assert ".." not in line, (
            f"path traversal in {manifest}: {line}"
        )
...
assert actual == expected, (
    "knowledge manifest differs from its certified Phase VI–VII order\n"
    f"expected: {expected}\n"
    f"actual:   {actual}"
)
PY
...
manifest = Path("dev/verification/manifests/genesis.manifest")
...
assert len(actual) >= len(constitutional_prefix), (
    "Genesis manifest is shorter than the certified constitutional prefix"
)

actual_prefix = actual[:len(constitutional_prefix)]

assert actual_prefix == constitutional_prefix, (
    "Genesis constitutional prefix has changed\n"
    f"expected prefix: {constitutional_prefix}\n"
    f"actual prefix:   {actual_prefix}"
)

extensions = actual[len(constitutional_prefix):]

for entry in extensions:
    assert entry.startswith("dev/verify_"), (
        f"invalid evolutionary verifier path: {entry}"
    )
    assert entry.endswith(".sh"), (
        f"evolutionary verifier must be a shell wrapper: {entry}"
    )
    assert entry not in constitutional_prefix, (
        f"constitutional verifier repeated as extension: {entry}"
    )

print(
    f"[INFO] Constitutional suites: {len(constitutional_prefix)}"
)
print(
    f"[INFO] Evolutionary extensions: {len(extensions)}"
)

for entry in extensions:
    print(f"[INFO] Extension: {entry}")
PY
