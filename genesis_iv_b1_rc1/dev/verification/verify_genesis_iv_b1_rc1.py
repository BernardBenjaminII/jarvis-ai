import hashlib, importlib, json, sys, tempfile, subprocess
...
def check(c,l):
    ...
    print('[PASS]',l)
def main():
    check(all((ROOT/p).is_file() for p in REQUIRED),'Canonical IV-B1 RC1 file set')
    ...
    with tempfile.TemporaryDirectory() as d:
        audit_root=Path(d); audit_output=audit_root/'audit.json'
        result=subprocess.run([sys.executable,'-m','core.integration.cli','audit','--repository-root',str(audit_root),'--output',str(audit_output)],cwd=ROOT,capture_output=True,text=True,check=False)
        check(result.returncode==0,'Audit CLI execution')
        check(audit_output.is_file(),'Audit CLI output file')
        json.loads(audit_output.read_text(encoding='utf-8'))
        check(True,'Audit CLI valid JSON')
    ...
    fp=hashlib.sha256(json.dumps(payload,sort_keys=True,separators=(',',':')).encode()).hexdigest(); print('[PASS] Deterministic IV-B1 RC1 fingerprint:',fp)
