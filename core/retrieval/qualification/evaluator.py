...
class QualificationEngine:
    ...
    def evaluate_candidate(self, query: str, candidate: EvidenceCandidate) -> QualifiedEvidence:
        text = " ".join(x for x in (candidate.title,candidate.subject,candidate.excerpt,candidate.source_path) if x)
        ...
        entity = phrase.score if phrase.phrases else lexical.score
    ...
    def _decide(self, lexical, phrase, subject, confidence, final):
        ...
        if phrase >= t.strong_phrase_override and final >= t.accept:
            return QualificationDecision.ACCEPTED, "accepted by strong phrase match"
