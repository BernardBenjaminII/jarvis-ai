...
const CONVERSATION_ENDPOINT = "/api/conversation/query";
const SESSION_KEY = "jarvis.executive.conversation.session";

function conversationSession() {
  let value = window.localStorage.getItem(SESSION_KEY);
  if (!value) {
    value = self.crypto?.randomUUID?.().replaceAll("-", "") || `${Date.now()}${Math.random()}`.replace(".", "");
...
function addMessage(role, content, trace = []) {
  ...
  label.textContent = role === "operator" ? "COMMANDER" : "JARVIS";
  ...
  if (trace.length) {
    ...
    trace.forEach((item) => {
      ...
      entry.textContent = `${item.stage}: ${item.detail}`;
      list.append(entry);
    });
    details.append(summary, list);
    article.append(details);
  }
  $("conversation-log").append(article);
...
async function submitConversation(event) {
  ...
  if (!question) return;
  addMessage("operator", question);
  input.value = "";
  $("send-command").disabled = true;
  $("conversation-state").textContent = "PROCESSING";
  $("conversation-state").className = "pill reasoning";
  $("conversation-detail").textContent = "JARVIS is processing the request…";
  try {
    const response = await fetch(CONVERSATION_ENDPOINT, {
      method: "POST",
      headers: {"Content-Type": "application/json", Accept: "application/json"},
      body: JSON.stringify({question, session_id: conversationSession(), mode: "full", channel: "text"}),
    });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.detail || `Conversation API: ${response.status}`);
    addMessage("jarvis", payload.answer || "No response returned.", payload.trace || []);
    $("conversation-state").textContent = String(payload.state || "completed").toUpperCase();
    $("conversation-state").className = `pill ${norm(payload.state || "complete")}`;
    $("conversation-detail").textContent = `${(payload.objectives || []).length} objective(s) compiled · session ${payload.session_id.slice(0, 8)}`;
    $("chat-api").textContent = "CONNECTED";
  } catch (error) {
    addMessage("jarvis", `Conversation link failure: ${error.message}`);
    $("conversation-state").textContent = "FAILED";
    $("conversation-state").className = "pill failed";
    $("chat-api").textContent = "DEGRADED";
  } finally {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("open-chat").addEventListener("click", () => $("conversation").scrollIntoView({behavior: "smooth", block: "start"}));
