...
from core.retrieval.semantic_index.service_scale import CorpusScaleSemanticService

p=argparse.ArgumentParser(description="Genesis X-B1.2 — Corpus-Scale Semantic Indexing & Adaptive Throughput")
...
if a.command in ("audit","status"):
    ...
    out=s.execute(limit=max(1,a.limit),batch_size=max(1,a.batch_size),report_every=max(1,a.report_every))
else:
    out=s.certify()
